  ...
  try {
    ...
    for (let i = 0; i < dataArray.length; i++) {
      ...
      if (dataArray[i].trim() === "1" && reachedClassification) {
        ...
        // TODO: Some TLE data has 8 or 9 strings in second line
        console.log(valueAfter, /[a-zA-Z]/.test(valueAfter));   
        currentTle += dataArray.slice(0, 18).join(""); /* For returning with the name of satellite along with all the TLE data */
        // currentTle = dataArray.slice(0, i + 1).join(""); /* For returning only the TLE data */
...
export function parseTleToJson(data: string): SatelliteData {
  try {
